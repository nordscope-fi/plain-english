...
The same argument decided how the coding agents are supported. Each agent gets a translation table in `src/agents/`, mapping its payload onto one canonical event and one `Decision` back onto its wire format. Nothing about deciding lives in a profile. That was affordable because Claude Code's hook contract turned into the shape the others copied. Copilot ships an explicit compatibility mode for it. Codex uses the same reply vocabulary, and Cursor uses the same event with different words. One linter per agent would have drifted the way five word lists did.
